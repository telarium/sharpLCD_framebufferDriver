...
char DISP = 132; // Pull to ground to turn off display - CSID0
char SCS = 128; // CS - CSI_PCLK for CHIP select. This may not be needed if the SPI driver does this anyway.
char VCOM = 133; // EXMODE vcom toggle - CSID1, although it doesn't seem like it's ever actually used.

int lcdWidth = LCDWIDTH;
int lcdHeight = 240;
int fpsCounter;

static int threshold = 4; // Necessary for fbcon
module_param(threshold, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP );
...
int thread_fn(void) 
{
    ...
    screenBufferCompressed[y*(50+4) + 1] = reverseByte(y);
    ...
    while (!kthread_should_stop()) 
    {
        ...
        for(y=0 ; y < 240 ; y++)
        {
            ...
            for(x=0 ; x<50 ; x++)
            {
                for(i=0 ; i<8 ; i++ )
                {
                    pixel = ioread8(info->fix.smem_start + (x*8 + y*400 + i)*3);
            ...
            {
                gpio_set_value(SCS, 1);
                // Allocating memory with vzalloc seems too slow...
